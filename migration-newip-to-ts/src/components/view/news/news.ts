import './news.css';
import { NewsStructure } from '../../../types/interfaces';

class News {
    public draw(data: NewsStructure[]): void {
        const news: NewsStructure[] = data.length >= 10 ? data.filter((_item, idx) => idx < 10) : data;
